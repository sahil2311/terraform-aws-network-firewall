locals {
  ...
  managed_rules = var.attached_managed_rules ? local.managed_rule_group_arns : concat([aws_networkfirewall_rule_group.block_domains.arn, aws_networkfirewall_rule_group.block_public_dns_resolvers.arn])
}
