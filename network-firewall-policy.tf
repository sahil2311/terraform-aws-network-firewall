locals {
  ...
  managed_rules = var.attached_stateful_managed_rules ? local.managed_rule_group_arns : concat(toset([aws_networkfirewall_rule_group.block_domains[0].arn, aws_networkfirewall_rule_group.block_public_dns_resolvers[0].arn]))
}
...
resource "aws_networkfirewall_firewall_policy" "aws_networkfirewall_firewall_policy" {
  ...
  firewall_policy {
    ...
    stateless_rule_group_reference {
      priority     = 20
      resource_arn = var.attached_stateless_icmp_blocked_rule ? aws_networkfirewall_rule_group.drop_icmp[0].arn : null
    }
